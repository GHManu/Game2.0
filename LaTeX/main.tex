...
\begin{titlepage}
  ...
  {\normalsize
    ...
    \textbf{Repository Game}: \href{https://github.com/GHManu/Game}{github.com/GHManu/Game}, \\[0.3em]
    \textbf{Repository principale}:  \href{https://github.com/GHManu/Game2.0}{github.com/GHManu/Game2.0}, \\[0.3em]
    \textbf{Nota}: I codici trascritti qua e i vari Diagrammi \textbf{non rappresentano l'intero progetto ma solo la parte significativa di esso}.
  \par}
  \vfill
...
\begin{tikzpicture}
    \begin{umlsystem}[x=4, y=0]{Game}
        \umlusecase[name=start]{Start}
        \umlusecase[name=move, y=-2]{Movement in the entire map}
        \umlusecase[name=sprint, x = 8, y = -2]{Sprint}
        \umlusecase[name=shoot, y=-4]{Attack}
        \umlusecase[name=win, y=-6]{Win}
        \umlusecase[name=loose,x = 8, y=-6]{Gameover}
        \umlusecase[name=close,x = 8, y=-7]{Close}
        \umlusecase[name=move_en, y=-8]{Movement in a part of the map}
        \umlusecase[name=shoot_en, y=-10]{Attack}
        \umlusecase[name=loose_en, y=-12]{Died}
        \umlusecase[name=win_en, x = 8, y=-12]{Player killed}
    \end{umlsystem}

    % Definizione degli Attori (posizionati fuori dal sistema)
    \umlactor[x=0, y=0]{Player}
    \umlactor[x=0, y=-10]{Enemy}

    % Associazioni tra Attori e Casi d'Uso
    \umlassoc{Player}{start}
    \umlassoc{Player}{move}
    \umlassoc{Player}{shoot}
    \umlassoc{Player}{win}
    \umlassoc{Enemy}{move_en}
    \umlassoc{Enemy}{shoot_en}
    \umlassoc{Enemy}{loose_en}
    


    % Relazioni tra Casi d'Uso (es. inclusione)
    \umlextend{sprint}{move}
    \umlextend{loose}{win}
    \umlextend{close}{win}
    \umlextend{win_en}{loose_en}
\end{tikzpicture}
}
\end{center}
...
\begin{figure}[htbp]
    \centering % Centra l'immagine nella pagina
    \includegraphics[width=1.1\linewidth]{ClassDiag.png}
